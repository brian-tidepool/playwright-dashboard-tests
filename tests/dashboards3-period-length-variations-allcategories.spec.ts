...
test.describe(
  "Dashboard Scenario 3 - Period Length Variations - All Categories",
  { tag: "@scenario3" },
  () => {
    ...
    async function cleanupDashboardPatients() {
      try {
        ...
        console.log(
          `Calling deletePatients with clinic: ${clinicId}, tag: ${tagId}`
        );
        ...
        console.log(
          "Successfully deleted patients using deletePatients function"
        );
      } catch (error) {
    ...
    test.beforeAll(async () => {
      ...
      credentials = {
        ...
        baseUrl:
          envVars.TIDEPOOL_BASE_URL || "https://qa2.development.tidepool.org",
      };
      ...
      if (
        ...
        throw new Error(
          "Missing Tidepool credentials. Please set TIDEPOOL_USERNAME, TIDEPOOL_PASSWORD, and TIDEPOOL_BASE_URL in your environment variables."
        );
      }

      if (!tagId || !clinicId) {
        throw new Error(
          "Missing TAG_SCENARIO3_ID or CLINIC_ID. Please set TAG_SCENARIO3_ID and CLINIC_ID in your environment variables."
        );
      }
    ...
    test("should verify dashboard data with one patient in each category across different summarization periods", async ({
      page,
    }) => {
      ...
      const waitTime = parseInt(
        process.env.WAIT_SUMMARY_CALCULATION_FINISH || "60000",
        10
      );
      ...
      try {
        console.log(
          "Verifying dashboard scenario 3 data with all categories across different summarization periods..."
        );
        ...
        await page.waitForSelector('text="Welcome To Tidepool"', {
          timeout: 15000,
        });

        // Find and click on the "a2 abc" workspace's "Go To Workspace" button
        const a2abcHeading = page.locator('h4:has-text("a2 abc")').first();
        const goToWorkspaceButton = a2abcHeading.locator(
          'xpath=following::button[normalize-space()="Go To Workspace"][1]'
        );
        ...
        await page.waitForSelector('text="TIDE Dashboard View"', {
          timeout: 15000,
        });
        await page.click('text="TIDE Dashboard View"');

        // Wait after clicking TIDE Dashboard View (configurable via environment variable)
        const waitAfterDashboardClick = parseInt(
          process.env.WAIT_AFTER_DASHBOARD_CLICK || "1000",
          10
        );
        console.log(
          `Waiting ${waitAfterDashboardClick}ms after clicking TIDE Dashboard View...`
        );
        ...
        try {
          ...
          await page.waitForSelector(
            'text="Select Patients to Display in the TIDE Dashboard"',
            { timeout: 5000 }
          );
        ...
        for (let i = 0; i < periodsToTest.length; i++) {
          ...
          if (i > 0) {
            ...
            await page
              .getByRole("button", {
                name: "Filter Patients Open dashboard config",
              })
            ...
            console.log(
              `Waiting ${waitAfterDashboardClick}ms after clicking Apply button...`
            );
            await page.waitForTimeout(waitAfterDashboardClick);
          }
          ...
          await page.waitForSelector('h3:has-text("TIDE Dashboard")', {
            timeout: 15000,
          });
          ...
          try {
            const summarizingContainer = page
              .locator('text="Summarizing"')
              .locator("..");
            if ((await summarizingContainer.count()) > 0) {
              const containerText =
                (await summarizingContainer.first().textContent()) || "";
              console.log(
                `Dashboard shows summarization period: '${containerText}'`
              );

              if (containerText.includes("of data")) {
                // Extract the period from "Summarizing24 hours of data" or "Summarizing 24 hours of data"
                const periodMatch = containerText.match(
                  /Summarizing\s*(.+?)\s+of data/
                );
                if (periodMatch && periodMatch[1]) {
                  ...
                  if (displayedPeriod === period) {
                    console.log(
                      `✅ Period validation passed: Found "${displayedPeriod}" as expected`
                    );
                  } else {
                    console.log(
                      `❌ Period validation failed: found "${displayedPeriod}", expected "${period}"`
                    );
                  }
                } else {
                  console.log(
                    `❌ Failed to extract period from container text`
                  );
                }
              }
            }
          ...
          if (displayedPeriod !== period) {
            throw new Error(
              `Period validation failed: expected "${period}" but found "${displayedPeriod}"`
            );
          }

          // Verify 1 patient row appears in each category (excluding Data Issues section)
          console.log(
            `Verifying 1 patient in each category for ${period} period...`
          );
          ...
          console.log(
            `Found ${
              categoriesToVerify.length
            } categories to verify: ${categoriesToVerify.join(", ")}`
          );

          if (categoriesToVerify.length === 0) {
            throw new Error(
              `No dashboard categories found for ${period} period. Dashboard may not have loaded properly.`
            );
          }
          ...
          for (const category of categoriesToVerify) {
            ...
            try {
              ...
              console.log(
                `Category ${category} still loading, waiting more...`
              );
              await page.waitForTimeout(5000);
            }
            ...
            if (hasCategoryData) {
              ...
              console.log(
                `Looking for patient table rows specifically in the ${category} section...`
              );

              const categorySectionHeader = page
                .locator(`text="${category}"`)
                .first();

              // Find the closest parent container that contains both the header and the table
              const categorySectionContainer = categorySectionHeader.locator(
                'xpath=ancestor::div[contains(@class, "section") or contains(@id, "section") or contains(@data-testid, "section")][1]'
              );
              ...
              if ((await categorySectionContainer.count()) > 0) {
                console.log(
                  `Found section container for ${category}, looking for table within it...`
                );
                categoryTable = categorySectionContainer
                  .locator('table[aria-label="peopletablelabel"]')
                  .first();
              } else {
                console.log(
                  `No specific section container found for ${category}, trying alternative approach...`
                );
                // Alternative: find the table that immediately follows the category text
                categoryTable = categorySectionHeader.locator(
                  'xpath=following::table[contains(@aria-label, "peopletablelabel")][1]'
                );
              }
              ...
              if ((await categoryTable.count()) > 0) {
                ...
                console.log(
                  `Found ${actualCount} patient rows in ${category} section table for ${period} period`
                );
                ...
                if (actualCount > 0) {
                  for (let j = 0; j < actualCount; j++) {
                    ...
                    if (patientName && patientName.includes("AllCategories")) {
                      console.log(
                        `  ✓ Patient name contains expected category identifier`
                      );
                    } else {
                      console.log(
                        `  ⚠ Patient name does not contain expected category identifier`
                      );
                    }
                  }
                }
                ...
                console.log(
                  `No patient table found directly in ${category} section`
                );

                // Check if there's a "no patients" message in this specific section
                const noDataMessage = categorySectionHeader.locator(
                  'xpath=following::*[contains(text(), "There are no patients that match your filter criteria")][1]'
                );
                if (await noDataMessage.isVisible()) {
                  actualCount = 0;
                  console.log(
                    `Found 'no patients' message in ${category} section, count is 0`
                  );
                } else {
                  // Last resort: check if this section shows a count instead of a table
                  const countText = categorySectionHeader.locator(
                    'xpath=following::*[contains(text(), "1") or contains(text(), "0")][1]'
                  );
                  if ((await countText.count()) > 0) {
                    ...
                    if (match) {
                      ...
                      console.log(
                        `Found count display for ${category}: ${actualCount}`
                      );
                    }
                  }
                }
              ...
              console.log(
                `Final patient row count in ${category} section for ${period} period: ${actualCount}`
              );
              ...
              if (actualCount === 0) {
                ...
                await page.screenshot({
                  path: `dashboard-allcategories-nodata-debug-${category.replace(
                    /[^a-zA-Z0-9]/g,
                    "-"
                  )}-${period.replace(" ", "-")}-${Date.now()}.png`,
                  fullPage: true,
                });
                console.log(
                  `Screenshot saved for debugging no-data state in ${category} section with ${period} filter`
                );

                throw new Error(
                  `Expected 1 patient row in ${category} section for ${period} period, but found 0 patient rows`
                );
              }

              expect(actualCount).toBe(1);
              console.log(
                `✓ Successfully verified 1 patient row in ${category} section for ${period} period`
              );
            } else {
              // Take a screenshot for debugging
              await page.screenshot({
                path: `dashboard-allcategories-nosection-debug-${category.replace(
                  /[^a-zA-Z0-9]/g,
                  "-"
                )}-${period.replace(" ", "-")}-${Date.now()}.png`,
                fullPage: true,
              ...
              console.log(
                `Screenshot saved for debugging no-section state for ${category} with ${period} filter`
              );

              // Test should fail if category section is not visible
              throw new Error(
                `Expected ${category} section to be visible for ${period} period, but section was not found`
              );
            }
          }
        }
        ...
        console.log(
          "✓ Verified all summarization periods (24 hours, 7 days, 14 days, 30 days)"
        );
        console.log(
          "✓ Successfully verified 1 patient in each category across all periods"
        );
        console.log(
          "✓ Successfully tested period filtering functionality with Apply button"
        );
      } catch (error) {
